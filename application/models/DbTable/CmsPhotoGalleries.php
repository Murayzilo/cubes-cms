...
class Application_Model_DbTable_CmsPhotoGalleries extends Zend_Db_Table_Abstract {
     ...
     * @return int The new ID of new photoGallery (autoincrement)
     */
    ...
    /**
     * 
     * @param nt $id ID of photoGallery to enable
     */
    ...
    public function updatePhotoGalleryOfOrder($sortedIds) {
    ...
    protected function processFilters(array $filters, Zend_Db_Select $select) {
        ...
        foreach ($filters as $field => $value) {
            switch ($field) {
                ...
                case 'title_search':
                    $select->where('title_ LIKE ?', '%' . $value . '%');
                    break;
